...
{
    if (!WaylandServer::self() || !m_renderDevice) {
        return;
    }
    ...
    const dev_t mainDevice = m_renderDevice->eglDisplay()->renderDevNode().value_or(scanoutDevice->deviceId());
    m_tranches.append({
        .device = mainDevice,
...
{
    RenderDevice *device = targetDevice == m_renderDevice->drmDevice()->deviceId()
        ? m_renderDevice
        : GpuManager::self()->compatibleRenderDevice(targetDevice);
    if (!device) {
        return false;
    }
